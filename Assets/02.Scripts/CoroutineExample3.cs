...
public class CoroutineExample3 : MonoBehaviour
{
    ...
    {
        // 여러 코루틴을 연속해서 실행할 경우 중첩 코루틴을 사용하지 말고 시퀀스 방식으로 해결
        yield return StartCoroutine(Ready_Coroutine(1f));
        yield return StartCoroutine(Start_Coroutine(2f));
        yield return StartCoroutine(End_Coroutine(3f));
    }

    private IEnumerator Ready_Coroutine(float second)
    {
        yield return new WaitForSeconds(second);
        Debug.Log($"{second}1초 대기");
